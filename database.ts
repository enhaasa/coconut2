...
export default class Database {
    ...
    public static async add(table: string, row: object, returnQuery: string | null = null) {
      return new Promise((resolve, reject) => {
        ...
        let query = `INSERT INTO ${table} (${keys}) VALUES (${values.map((_, index) => ("$"+(index+1))).toString()})`;

        if (returnQuery) {
          query += ` RETURNING "${returnQuery}"`;
        }
        
        this.query(query, [...values], (err) => {
          if (err) {
            console.log(err);
            reject(err);
          } 
     ...
     */
    public static async update(table:string, key, value, con_key, con_val) {
      return new Promise(async (resolve, reject) => {
        ...
        this.query(
            ...
            (err, result) => {
                if (err) {
                  ...
                  resolve('Successfully updated the record.');
                }
            }
        );
      });
